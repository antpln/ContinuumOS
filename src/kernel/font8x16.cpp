...
namespace
{
// 8x8 bitmap font covering ASCII 0x20-0x7E (public domain, derived from font8x8_basic).
...
constexpr size_t SOURCE_FONT_HEIGHT = 8;
constexpr size_t TOP_PADDING_ROWS = 2;
constexpr size_t BOTTOM_PADDING_ROWS = 2;

uint8_t g_font_cache[95][FONT_HEIGHT];
bool g_font_cache_ready = false;

void ensure_font_cache()
{
	if (g_font_cache_ready)
	{
		return;
	}

	const size_t padding_total = TOP_PADDING_ROWS + BOTTOM_PADDING_ROWS;
	const size_t drawable_rows = FONT_HEIGHT > padding_total ? FONT_HEIGHT - padding_total : 0;
	const size_t drawable_denominator = drawable_rows > 1 ? (drawable_rows - 1) : 1;

	for (size_t ch = 0; ch < 95; ++ch)
	{
		for (size_t row = 0; row < FONT_HEIGHT; ++row)
		{
			uint8_t row_bits = 0;

			if (drawable_rows == 0 || row < TOP_PADDING_ROWS || row >= FONT_HEIGHT - BOTTOM_PADDING_ROWS)
			{
				g_font_cache[ch][row] = 0;
				continue;
			}

			const size_t scaled_row = ((row - TOP_PADDING_ROWS) * (SOURCE_FONT_HEIGHT - 1)) / drawable_denominator;

			for (size_t col = 0; col < FONT_WIDTH; ++col)
			{
				const uint8_t column_bits = FONT8x8_BASIC[ch][col];
				// FONT8x8_BASIC is column-major with the least significant bit at the top.
				const bool pixel_on = ((column_bits >> scaled_row) & 0x1u) != 0;
				if (pixel_on)
				{
					row_bits |= static_cast<uint8_t>(0x80u >> col);
				}
			}

			g_font_cache[ch][row] = row_bits;
		}
	}

	g_font_cache_ready = true;
}
} // namespace
...
const uint8_t *glyph_for(char ch)
{
	...
	return g_font_cache[static_cast<size_t>(ch - 32)];
}
